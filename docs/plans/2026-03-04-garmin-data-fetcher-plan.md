# Garmin Data Fetcher Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fetch all available data from Garmin Connect API and save raw JSON responses to `projects/coach/data/garmin/`.

**Architecture:** Single script `fetch_garmin.py` that authenticates via garminconnect library, then systematically calls every available API endpoint. Data organized into subdirectories by category (activities, daily, weekly, profile, devices, gear, etc.). Incremental mode skips already-fetched data.

**Tech Stack:** Python 3.10+, garminconnect (pip)
...
**Files:**
- Create: `projects/coach/data/garmin/.gitkeep`
- Modify: `projects/coach/.gitignore`

**Step 1: Create data directory structure**

```bash
mkdir -p /mnt/d/prg/plum/projects/coach/data/garmin
touch /mnt/d/prg/plum/projects/coach/data/garmin/.gitkeep
```

**Step 2: Update .gitignore**

Add to `projects/coach/.gitignore`:
```
data/garmin/.tokens/
data/garmin/activities/
data/garmin/daily/
data/garmin/weekly/
data/garmin/profile/
data/garmin/devices/
...
data/garmin/goals/
data/garmin/badges/
data/garmin/challenges/
data/garmin/workouts/
data/garmin/blood_pressure/
data/garmin/weight/
data/garmin/progress/
```
...
```bash
git add projects/coach/.gitignore projects/coach/data/garmin/.gitkeep
git commit -m "chore(coach): add garmin data directory and gitignore"
```

---

### Task 2: Auth and skeleton script

**Files:**
- Create: `projects/coach/fetch_garmin.py`

**Step 1: Write the script skeleton with auth**
...
**Step 2: Run to verify auth works**

```bash
cd /mnt/d/prg/plum/projects/coach && python fetch_garmin.py
```

Expected: prompts for credentials, authenticates, then fails on undefined fetch_* functions. That's fine — confirms auth works.

**Step 3: Commit**

```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add garmin fetcher skeleton with auth"
```

---

### Task 3: Profile, devices, gear, badges, goals, workouts

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add all the "fetch once" functions**
...
def fetch_gear(garmin: Garmin) -> None:
    ...
    # Need userProfileNumber from profile
...
```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add profile/devices/gear/badges/goals/workouts fetch"
```

---

### Task 4: Activities list fetch

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add activities list fetch**
...
def fetch_activities(garmin: Garmin, today: date, full: bool = False) -> list:
    ...
    # Fetch from Garmin — either incremental or full
    ...
    if newest_date and not full:
        # Merge: keep existing, add only truly new ones
...
```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add incremental activities list fetch"
```

---

### Task 5: Per-activity detail fetch

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add per-activity detail fetch**
...
def fetch_activity_details(garmin: Garmin, activities: list, full: bool = False) -> None:
    ...
    for i, activity in enumerate(activities):
        ...
        # Skip if all files exist (incremental mode)
        ...
        time.sleep(1)  # rate limit between activities
...
```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add per-activity detail fetch with incremental skip"
```

---

### Task 6: Daily data fetch

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add daily data fetch**
...
def fetch_daily(garmin: Garmin, activities: list, today: date, full: bool = False) -> None:
    ...
    # Determine date range: earliest activity to today
    activity_dates = []
    for a in activities:
    ...
    # Build set of dates to fetch: all activity dates + last 90 days
...
```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add daily wellness data fetch"
```

---

### Task 7: Weekly and range data fetch

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add weekly and range data fetch**
...
def fetch_weekly(garmin: Garmin, today: date, full: bool = False) -> None:
    ...
    # get_weekly_steps and get_weekly_stress take (end_date, weeks)
    # get_weekly_intensity_minutes takes (start_date, end_date)
...
```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): add weekly and range data fetch"
```

---

### Task 8: Progress display and final polish

**Files:**
- Modify: `projects/coach/fetch_garmin.py`

**Step 1: Add a summary counter at the end of main()**

In `main()`, after all fetch calls, add:
...
Expected: authenticates, fetches all data categories, prints progress, saves JSON files.

**Step 3: Verify incremental mode**

```bash
cd /mnt/d/prg/plum/projects/coach && python fetch_garmin.py
```

Expected: second run skips most fetches, finishes quickly.

**Step 4: Verify --full flag**

```bash
cd /mnt/d/prg/plum/projects/coach && python fetch_garmin.py --full
```

Expected: re-fetches everything.

**Step 5: Commit**

```bash
git add projects/coach/fetch_garmin.py
git commit -m "feat(coach): finalize garmin data fetcher with summary stats"
```
...
**Files:**
- Modify: `projects/coach/CLAUDE.md`

**Step 1: Add fetch_garmin.py documentation to CLAUDE.md**

Add a section covering:
- The fetch command and its flags
...
```bash
git add projects/coach/CLAUDE.md
git commit -m "docs(coach): add garmin fetcher to CLAUDE.md"
```
