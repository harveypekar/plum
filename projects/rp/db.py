...
async def list_templates() -> list[dict]:
    pool = await get_pool()
    rows = await pool.fetch(
        "SELECT id, name, content, active, created_at::text, updated_at::text "
...
async def get_active_template() -> dict | None:
    pool = await get_pool()
    row = await pool.fetchrow(
        "SELECT id, name, content, active, created_at::text, updated_at::text "
        "FROM rp_prompt_templates WHERE active = TRUE LIMIT 1"
    )
    return dict(row) if row else None


async def activate_template(template_id: int) -> dict | None:
    pool = await get_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute("UPDATE rp_prompt_templates SET active = FALSE WHERE active = TRUE")
            row = await conn.fetchrow(
                "UPDATE rp_prompt_templates SET active = TRUE, updated_at=NOW() "
                "WHERE id = $1 RETURNING id, name, content, active, "
...
async def deactivate_all_templates() -> None:
    pool = await get_pool()
    await pool.execute("UPDATE rp_prompt_templates SET active = FALSE WHERE active = TRUE")
...
async def create_template(name: str, content: str) -> dict:
    ...
    row = await pool.fetchrow(
        ...
        "VALUES ($1, $2) RETURNING id, name, content, active, "
        "created_at::text, updated_at::text",
        name, content,
    )
    return dict(row)
...
async def update_template(template_id: int, name: str, content: str) -> dict | None:
    ...
    row = await pool.fetchrow(
        ...
        "updated_at=NOW() WHERE id=$1 RETURNING id, name, content, active, "
        "created_at::text, updated_at::text",
